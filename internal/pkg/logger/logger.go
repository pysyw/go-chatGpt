...
func createZapLog() *zap.Logger {
	// 开启 debug
	if config.GLOBAL_CONFIG.ENV == "Debug" {
	...
	filename := filepath.Join(config.GLOBAL_CONFIG.Logger.STATIC_BASE_PATH, "/logs/", config.GLOBAL_CONFIG.Logger.FILENAME)
	var writer zapcore.WriteSyncer
	if config.GLOBAL_CONFIG.Logger.DEFAULT_DIVISION == "size" {
		// 按文件大小切割日志
...
func getRotateWriter(filename string) io.Writer {
	cMaxAge, _ := strconv.Atoi(config.GLOBAL_CONFIG.Logger.DIVISION_TIME.MAX_AGE)
	maxAge := time.Duration(cMaxAge)
	cRotationTime, _ := strconv.Atoi(config.GLOBAL_CONFIG.Logger.DIVISION_TIME.RotationTime)
	rotationTime := time.Duration(cRotationTime)
